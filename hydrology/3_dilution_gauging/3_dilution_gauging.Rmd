...
**Question**: *Look at equation 1 to determine how mass loss of tracer (NaCl) to the GW system would influence your estimate of Q?* 
...
And a cleaner data set for use with R:

[here](https://www.dropbox.com/s/m0tqrunod6b9e9u/corrected_conductivity_working.csv?dl=0)

<!-- If you wish to work in R, this [example code](https://www.dropbox.com/sh/082lpm94i4kwv3t/AABiRSMTD3Q6QyO2YHplJo2ja?dl=0) will help you -->

## Breakthrough Curve Analysis

Use the provided conductivity breakthrough curve timeseries data to calculate discharge with the dilution gauging method following the directions below:


a.	It is useful to convert the timestamp to time since injection (in second) for calculations. Subtract the Start Time from each Timestamp then convert from fractional days to seconds (hint: there are 86400 seconds/day).

b.	When working with tracers you first need to correct for background (i.e. pre-arrival) levels. To do this plot the conductivity data vs. time and determine the background conductivity. You want to capture an average pre-arrival conductivity value. However, it takes some time for conductivity sensors to adapt to local stream conditions. This is because of the temperature influence on conductivity. It takes some time for the conductivity to equilibrate to the temperature of the stream, and during that time the conductivity data are not accurate. Once the sensor has equilibrated, the data are usable. Keep in mind that when we are using conductivity data we generally want to use data that are temperature compensated (i.e., corected to 25 degrees C). Data from Campbell data loggers will by default call this temperature corrected data "COND". When you are determining your background conductivity you should not include data that are rapidly increasing or decreasing pre-arrival, rather pick a relatively stable pre-arrival period. Once you have decided on the background value, subtract this number from all of the recorded conductivity values to provide background corrected values. Note that when you subtract the average background value from the BTC data you may return negative values. You will need to remove any negative values.

c.	Now that you have background corrected conductivity you will need to convert that to NaCl using a concentration factor of 2. Keep in mind that the NaCl values prior to arrival of the tracer should be near zero (because this is background corrected). To convert from conductivity to NaCl (mg/L) use the following equation:

![](images/equation_2.jpg)

d.	Graph the NaCl (mg/L) breakthrough curve (i.e., time since injection (s), vs background corrected NaCl values).
...
f.	Using the breakthrough curve (BTC) determine Q. If you do the unit analysis you will see this yields Q in L/s. Remember the BTC needs to be background corrected before you integrate it.

![](images/equation_4.jpg)

g.	From the BTC calculate tracer velocities. First, determine the time of the BTC peak. Calculate the time (in seconds) from injection to the peak of the BTC. The distance of injection divided by time to peak is the modal velocity.
...
i.	Find the time (in seconds) from injection time to 5%, 50%, and 95% arrival times. The reach length (distance from injection site to monitoring site) divided by the time to 5, 50, and 95% are the 5%, 50% (median), and 95% velocities.
...
Use the data we collected in the field work section to answer the following questions

**Dilution Gauging Questions (45 pts)**

a.	Provide a table with the following information for each of the 3 dilution gauging injections that we completed in the field (5 pts):
...
b.	Explain what is responsible for the differences in velocities (i.e modal vs. 5% vs. 50% vs. 95%) in the injection your group completed? (10 pts)


c.	Explain what is responsible for the differences in velocities between the 3 injections we completed? (10 pts)

d. Do the discharge values vary between the injections? Explain why these values may change? (5 pts)

d.	How would mass loss affect your calculation of Q and why? (5 pts)

e.	Provide a figure with the three normalized integrated slug BTCs with a y-scale that varies from 0-1. Explain differences in the shape of these curves (these are called cumulative density functions, CDFs) in terms of channel morphology and hydrodynamics.  (10 pts)

g.	Imagine 2 systems - a natural stream and a cement lined straight channel. Predict what the BTCs of a slug injection in each would look like. You can draw an illustration to demonstrate if you'd like. (5 pts)
