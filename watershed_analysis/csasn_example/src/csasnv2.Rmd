...
```{r}
data <- read_csv(here('/data', 'csasn_vf_metab_env_p_values_NA.csv'))

zero <- read_csv(here('/data', 'csasn_vf_metab_env_zero.csv')) #this includes the 0 uptake measurements
```
...
ggplot(data, aes(x = ER, y = NH4_Vf, shape = Site, color = Site)) +
  geom_point(size = 3) +
  theme_few() +
  ...
  stat_smooth(aes(group = 1), method='lm', color = 'black', lty=2) +
...
ggplot(data, aes(x = GPP, y = NH4_Vf, shape = Site, color = Site)) +
  geom_point(size = 3) +
  theme_few() +
  theme(legend.title=element_blank()) +
  ylab(expression(NH[4]-N~V[f-amb]~(mm/min))) +
